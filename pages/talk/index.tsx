    ...
    return (
        <Layouts>
            <div className="py-7 px-5">
                {data && data.map((e) => (
                    <Link href={`talk/${e.id}`} key={e.id} className="w-full h-full block ">
                        <ul className="h-20 flex items-center lg:justify-normal lg:h-24">
                            {e.image ? <Image src={e.image} alt={"image"} /> : <Image src={user} alt="image" width={55} height={55} />}
                            <div className="pl-5">
                                <p className="md:text-2xl">{e.name}</p>
                                <p className="text-left md:text-xl">{e.text.length > 35 ? <>{e.text.substring(0,35)}...</>:<>{e.text}</>}</p>
                            </div>
                        </ul>
                    </Link>
                ))}
