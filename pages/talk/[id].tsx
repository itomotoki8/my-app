...
const User = () => {
  ...
  useEffect(() => {
    ...
    axios
      .post("http://localhost:8000/api/get_talks", { uid: uid })
  ...
  return (
    <div className="bg-sky-300 min-h-screen">
      ...
      <div className="md:mx-10 md:text-xl">
        <div className="px-2 pt-5">
          <div className="flex mb-5 gap-1">
            ...
            <p className="bg-white px-2 py-1 rounded-xl max-w-[80%]">
              user{id}のテキストです。
            </p>
          </div>

          <div className="flex justify-end">
            <p className="bg-green-200 p-2 rounded-xl  max-w-[80%]">
              userのテキストです。
            </p>
          </div>
        </div>
      ...
      <div className="shadow-2xl shadow-black bg-white h-20 fixed bottom-0 w-full flex items-center justify-center">
        <textarea
          className="border border-slate-800 rounded-lg w-3/4 p-2 leading-5 resize-none"
          placeholder="Aa"
        />
